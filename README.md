...
+src/   :フロントサイドでいじる部分
  ...
  +package.json :使用するパッケージとその使い方を記述しているファイル。
```
...
How toで登場したコマンド`npm run ~`はnpm記録ファイルである`package.json`に記述しているscript。
そのため、知識がないならいじらなくて良いし、shellコマンドに自信あるなら編集して改良しても良い。
